...
SELECT
	YEAR(website_sessions.created_at) AS yr,
	MONTH(website_sessions.created_at) AS mo,
	COUNT(DISTINCT CASE WHEN device type = 'desktop' THEN website_sessions.website_session_id ELSE NULL END) AS desktop_sessions,
	COUNT(DISTINCT CASE WHEN device type = 'desktop' THEN orders.order_id ELSE NULL END) AS desktop_orders,
	COUNT(DISTINCT CASE WHEN device type = 'mobile' THEN website_sessions.website_session_id ELSE NULL END) AS mobile_sessions,
	COUNT(DISTINCT CASE WHEN device type = 'mobile' THEN orders.order_id ELSE NULL END) AS brand_orders 
FROM website_sessions
	LEFT JOIN orders
		ON orders.website_session_id = website_sessions.website_session_id
WHERE website_sessions.created_at < '2012-11-27'
...
/*
4. I'm worried that one of our more passimistic board members may be concerned about the large % of traffic from Gsearch.
Can you pull monthly trends for Gsearch, alongside monthly trends for each of our other channels?
*/

--first, finding the various utm sources referers to see the traffic we're getting
...
nonbrand sessions and revenue since then to calculate incremantal value)
*/
...
FROM nonbrand_test_session_w_landing_pages
LEFT JOIN orders
	ON orders.website_session_id = nonbrand_test_session_w_landing_pages.website_session_id;
...
-- .0139 for /home, vs .0406 for /lander-1
...
-- X .0087 incremental conversion = 202 incremantal orders since 7/29
...
FROM website_sessions
	LEFT JOIN website_pageviews
		ON website_sessions.website_pageview_id = website_pageviews.website_session_id
...
SELECT
	CASE
		WHEN saw_homepage = 1 THEN 'saw_homepage'
		WHEN saw_custom_lander = 1 THEN 'saw_custom_lander'
		ELSE'uh oh ... checklogic'
	END AS segment,
	COUNT(DISTINCT CASE WHEN product_made_it = 1 THEN website_session_id ELSE NULL END)/COUNT(DISTINCT website_pageview_id) AS lander_click_rt,
